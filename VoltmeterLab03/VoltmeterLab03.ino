...
#include <LiquidCrystal.h> // Includes LiquidCrystal library which is used to program with lcd.
...
void setup() {
  ...
  lcd.begin(16, 2);                                            // Initialize the LCD with 16 columns and 2 rows.The first line and colunm are line and colunm 0.
...
void loop() {
 ...
 float voltage = (voltX/1000);                                 // Divides voltX by 1000 to get value in volts.
  ...
  // had to convert to milli volts cause map function cannot be a float varriable so to get decimal places must first convert value to milli then to volts.
